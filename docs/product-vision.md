...
| Relational experiences are supported | Mostly. An employer can offer job shadows, portfolio reviews, group sessions, and one-to-one mentoring; students see who is offering; and the introduction is now recorded, so a mentorship that happened is on the record and a mentor's declared capacity is checkable. The outcome it counts toward now exists, and an outcome can be recorded against a learner with no application at all — what is still missing is any link from a *particular* introduction to the outcome that followed it. |
...
| Minor status governs consent and hours | Partly. Consent is now recorded against the institution whose records it covers, with who signed captured rather than guessed — which is the half that holds whatever a district's counsel says about age. Still not modelled: the school a learner *attends*, so nothing can work out whose signature was required, and nothing distinguishes a dual-enrolled sixteen-year-old from a college sophomore for the hour and occupation limits that govern paid work under 18. |
| Measures transitions into employment | **Built.** A follow-up observation records where a learner went after an experience — hired by the host, employed in the region, employed elsewhere, continued education, entered training, or still looking — and the administrator's console reports regional employment beside the follow-ups nobody has done. What is not yet built is a fixed follow-up interval: workforce reporting measures at set quarters after exit, and this records whenever somebody asks. |
...
| Committed-versus-unspent allocation | Built, and now over a ledger rather than a single figure. A market carries no money of its own: a board's wage subsidy, a foundation's grant against the cost of internship credit, a college's fee waiver and a transport fund are all sources with balances, and every screen derives from them. Allocations change through an audited write with a reason, because a supplemental award or a rescission is ordinary administration. **Not yet modelled:** program-year rollover (Q24). |
