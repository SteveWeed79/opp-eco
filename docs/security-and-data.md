...
| Loading fixtures destroys only the demonstration | `scripts/seed.mjs`, `markets.is_demo_data` | `db:seed` truncated, so a refresh of the demo data destroyed the administrator `db:admin` had just stood up, silently. It now clears only the markets flagged as the demonstration: a real market, its learners, its money and every administrator are not "preserved" from it, they are never named by any statement it runs. Two things it deliberately cannot do, both from the audit guard: `audit_events` is append-only by trigger, so a demonstration's history accumulates rather than resetting and the seed declines to write a second copy of it — and because audit rows reference `markets` and `users` `ON DELETE RESTRICT`, neither can be deleted once the application has written one, so the fixtures upsert them in place. The old `TRUNCATE` was getting around that trigger rather than respecting it |
