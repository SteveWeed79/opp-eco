...
| Loading fixtures cannot take the only way in | `scripts/seed.mjs` | `db:seed` truncates, and an administrator cannot be created through the product — so a refresh of the demo data used to destroy the account that `db:admin` had just stood up, silently. It now carries administrators through the truncate with their passwords and authenticators, which is safe for that role alone because `admin_is_cross_market` anchors them to no market and no organization. Their sessions are not carried: a re-seed is exactly when a credential issued against the old rows should stop working. Anything else it did not write makes it **refuse** and name the tables — including a region definition, where a board's WIOA county list is real, hand-gathered data recorded against a seeded market |
