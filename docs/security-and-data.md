...
| An invented figure cannot pass as a real one | `markets.is_demo_data`, `scripts/seed.mjs`, `src/data/postgres/markets.test.ts` | The demonstration is data rather than a deployment mode, so the claim "this is fictional" lives on the rows it is about. One flag on a market covers every table, because they all carry `market_id`. **The fake is flagged and the real is the default**, so forgetting it hides the demo instead of exposing a learner — the opposite polarity fails silently and this product holds minors' education records. Nothing in the application can set it: `markets` has no write path outside the operator scripts, so a bug or a stolen administrator password cannot mark a real programme fictional. `markets.test.ts` fails if such a path appears, and pins the polarity so the column's meaning cannot be inverted quietly |
| Loading fixtures destroys only the demonstration | `scripts/seed.mjs`, `markets.is_demo_data` | `db:seed` truncated, so a refresh of the demo data destroyed the administrator `db:admin` had just stood up, silently. It now clears only the markets flagged as the demonstration: a real market, its learners, its money and every administrator are not "preserved" from it, they are never named by any statement it runs. The old `TRUNCATE` was getting around the audit trigger rather than respecting it — truncation does not fire row triggers — which is why scoping the seed turned a silent bypass into a visible error. Migration 0019 grants the one exemption that makes the demonstration rebuildable: a DELETE, never an UPDATE, of a row whose market is flagged `is_demo_data`. A real programme's audit trail stays exactly as immutable, and the application cannot reach the exemption because it cannot write `markets`. `markets` and `users` still cannot be deleted at all, so the fixtures upsert them |
| A visitor cannot write | `src/auth/visitor.ts`, `src/data/backend.ts` | A signed-out caller reaching a portal is handed the demonstration's own account, so the prototype stays walkable from a bare link. On the demonstration's own deployment that account may write — clicking through it is the point. On a deployment that authenticates, it may not: the same write would be an anonymous one against a database holding real programmes. Refused in the store rather than in the eleven action files, for the reason `ReadOnlyError` is: that is the only layer every write passes through, including a Server Action invoked straight from a fetch |
