...
| **Q23b** | ~~**On what interval?**~~ **Answered.** The 2nd and 4th calendar quarter after the quarter a placement ended in, measured from `exitedOn` rather than `statusSince`. The queue asks for an open window and stops asking for one that closed unanswered; those are reported as missed | Calendar quarters rather than elapsed months, so two cohorts are measured against the same calendar rather than their own end dates. Whether the board reports this programme on workforce quarters or not, the platform fixes the clock rather than each market choosing — the report's value is cross-town comparability, and a clock that varies by market has none |
