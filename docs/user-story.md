...
**[Q22 — settled]** Who initiates a pairing? **The college or an administrator introduces; the student does not ask and the employer does not accept.** The alternative — a request queue a student starts — was refused on the ground the rest of this model rests on: mentorship is the one form with no supervisor, no timesheet and no board interview standing between an adult and a student, so the first contact runs through the party that knows both. The administrator can introduce for the same reason they can do anything else here: a market whose college has not acted is what an operator exists to unstick, and that is the operator's job rather than an override.

The cost of that choice is the one named when it was open — it is a step nobody is prompted to take — and it is paid down by making the college's mentor list a queue with places on it rather than a list of names: an offer now shows how many of its declared places are free, and an introduction that nobody closes is visible as work in flight.

A pairing is recorded (`mentorship_pairings`), so declared capacity is reconciled against mentorships actually running — a live introduction spends a place, and closing one gives it back. Closing requires a note from whichever party records it, because "it did not happen" with no reason tells a college nothing about whether to try that mentor again, and "it happened" with no reason is the only evidence the platform will ever hold that a mentorship took place. **Still open:** whether a completed mentorship should count toward the employment outcome directly, which waits on that outcome being modelled at all.
...
| **Q23a** | ~~**Who may record an outcome?**~~ **Answered.** Three channels, each bounded to what its party can know: the college and the administrator record any outcome, a learner records their own, and the employer answers what it decided about its own headcount through `HostOffer` — where an accepted offer writes the outcome too. An employer still cannot file an outcome directly, because continued education, still looking, or a job somewhere else is hearsay from where it sits | The bottleneck was a college phoning fourteen people who have left town. Self-report carries a bias that runs one way — people who landed a good job answer — so `source` keeps a self-report distinguishable from a verified note, and the administrator's chase queue turns silence into a phone call rather than a gap in a chart |
| **Q23b** | **On what interval?** Still open, and the half with the clock running. Workforce reporting measures at fixed quarters after exit rather than whenever somebody asks — the 2nd and the 4th quarter after the exit quarter, counted in calendar quarters rather than elapsed months | Nobody can be phoned eighteen months later and asked where they were at three months. A late decision does not delay the report; it permanently shortens the series. If the board does not report this programme on workforce quarters, the argument for matching them is the report's cross-town comparability rather than compliance — which still means the platform fixes the clock rather than each market choosing |
