...
| **Q23** | **Who may record an outcome, and on what interval?** The college does it today. The learner and the employer each know things it does not — an employer is the only party that knows it made a hire — and workforce reporting measures at fixed quarters after exit rather than whenever somebody asks | Decides how much of the follow-up queue ever gets worked, and whether the resulting figure is comparable to the ones a board already reports. A college chasing fourteen learners by phone is the bottleneck; a self-report and an employer's confirmation are the two cheapest ways past it, and each needs a rule about what it may claim |
