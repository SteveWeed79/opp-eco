  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-20">
      <div className="bg-white rounded-2xl border border-brand-100 shadow-sm p-8">
        <span className="w-11 h-11 rounded-2xl bg-brand-50 text-brand-600 flex items-center justify-center mb-5">
            ...
            {DESTINATIONS.map((destination) => (
              <li key={destination.href}>
                <Link
                  ...
                  className="inline-block bg-white text-ink-950 border border-ink-200 font-semibold px-4 py-2 rounded-xl text-sm hover:border-brand-500 hover:text-brand-600 transition-colors"
                >
                  {destination.label}
                </Link>
              </li>
